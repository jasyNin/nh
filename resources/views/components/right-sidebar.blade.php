    ...
    @if($isTagsPage)
        <div class="card mb-4 border-0 ">
            <div class="card-header bg-transparent border-0 py-3">
                <h6 class="mb-0 fw-light">Популярные теги</h6>
            </div>
            <div class="card-body pt-0">
                <div class="d-flex flex-wrap gap-2">
                    @foreach($popularTags->take(6) as $tag)
                        <a href="{{ route('tags.show', $tag) }}" class="badge bg-light text-dark text-decoration-none px-3 py-2 rounded-pill">
                            #{{ $tag->name }}
        ...
        @if(count($recentAnswers) > 0)
            <div class="card border-0 ">
                <div class="card-header bg-transparent border-0 py-3">
                    <h6 class="mb-0 fw-light">Последние ответы</h6>
                </div>
                    ...
                    @foreach($recentAnswers->take(3) as $answer)
                        <a href="{{ route('posts.show', $answer->post) }}" class="list-group-item list-group-item-action border-0 py-3">
                            <div class="d-flex justify-content-between align-items-center">
                                <h6 class="mb-1 fw-light">{{ Str::limit($answer->post->title, 40) }}</h6>
